...
import           Codec.Midi                   (Channel, Key, Message (..),
                                               Midi (..), Time, Track, Velocity,
                                               isTrackEnd, toAbsTime,
                                               toRealTime, toSingleTrack)
import           Control.Concurrent
import           Control.Concurrent.STM.TChan
import           Control.Exception            (finally)
import           Control.Monad.STM            (atomically)
import           Data.IORef
import           Sound.PortMidi               (DeviceID, DeviceInfo (..),
                                               PMError (..), PMEvent (..),
                                               PMMsg (PMMsg), PMStream,
                                               PMSuccess (..), close,
                                               countDevices, decodeMsg,
                                               encodeMsg,
                                               getDefaultInputDeviceID,
                                               getDefaultOutputDeviceID,
                                               getDeviceInfo, getErrorText,
                                               initialize, openInput,
                                               openOutput, readEvents,
                                               terminate, time, writeShort)
