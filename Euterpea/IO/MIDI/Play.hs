...
import Codec.Midi
    ( Channel,
      FileType,
      Message(ProgramChange, KeyPressure, ChannelPressure, PitchWheel,
              TempoChange, ControlChange, NoteOn, NoteOff),
      Midi(Midi),
      Time,
      TimeDiv(..) )
...
import Euterpea.IO.MIDI.MEvent
    ( MEvent(eDur, eInst, eTime, ePitch, eVol), perform, perform1 )
...
import Sound.PortMidi
    ( getDefaultOutputDeviceID, initialize, terminate )
